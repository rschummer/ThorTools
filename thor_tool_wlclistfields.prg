...
* ------------------------------------------------------------------------------
* 06/19/2011  Richard A. Schummer     1.2      Sync changes made by Jim Nelson
*                                              for the Thor Repository
* ------------------------------------------------------------------------------
* 08/14/2011  Richard A. Schummer     1.3      Sync changes made by Jim Nelson
*                                              for current registration properties
* ------------------------------------------------------------------------------
* 11/03/2014  Richard A. Schummer     1.4      Prompt developer for cursor or table
*                                              if nothing open in current workarea
* ------------------------------------------------------------------------------
* 09/20/2015  Richard A. Schummer     1.5      Updated Thor registration information 
...
IF PCOUNT() = 1 AND 'O' = VARTYPE(m.txParam1) AND 'thorinfo' = LOWER(m.txParam1.Class)
   WITH m.txParam1
      ...
      * Optional
      .Description   = "Details the data in a cursor in the same format as a EDIT command does instead of a columnar BROWSE" 
...
DO CASE
   ...
   OTHERWISE
		...
		*  More notes here:	http://vfpx.codeplex.com/wikipage?title=PEM%20Editor%20Tools%20Object
...
IF EMPTY(ALIAS()) OR m.llFailedToSelectCursor
   ...
   ELSE
      TRY 
         USE (lcFile) IN 0 SHARED AGAIN ALIAS __ColListing
         DO ToolCode WITH txParam1, "__ColListing", tlModifyFile 
         USE IN (SELECT("__ColListing"))
   ...
   ENDIF 
ELSE
   DO ToolCode WITH txParam1, tcCursorName, tlModifyFile  
ENDIF 
...
*
*  METHOD DESCRIPTION:
*    This method runs the code for this particular tool.
*
*  INPUT PARAMETERS:
*    txParam1 = unknown, parameter passed in by default from Thor
*    tcCursorName = character, name of cursor if you have it.
*    tlModifyFile = logical, determines if modifying the result set text file
* 
*  OUTPUT PARAMETERS:
*    None
* 
********************************************************************************
PROCEDURE ToolCode(txParam1, tcCursorName, tlModifyFile)
...
TRY 
   this.nFields = AFIELDS(this.aFieldList)
